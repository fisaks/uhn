...
| `adapters` | Edge → Master | Configured infrastructure adapters (retained) | `{ adapters: [{ name, type }] }` |
...
**Device availability:**
Z2M publishes `zigbee2mqtt/{device}/availability` → transport publishes to `device/{name}/availability` on UHN MQTT (retained). Only for config-listed devices. Z2M bridge state (`zigbee2mqtt/bridge/state`) is also published as `device/{adapterName}/availability`. IHC drivers publish availability on connect (online) and disconnect/error (offline). All device availability feeds into UHN health notifications on the master.
