...
def test_when_something_external_change_input(rtu_sim_client, mqtt_watcher,io_kitchen):
    
    bit = random.randint(0, 7)
    s = given(rtu_sim_client, mqtt_watcher)

    (
        s.device(io_kitchen).and_input(bit).is_off().done()
            .resync() 
            .wait_for_expected_initial_states()
                .when_input_is_set_on(io_kitchen, bit)
                .thenDeviceInputBitIs(io_kitchen, bit, 1, timeout=3.0)
                .when_input_is_set_off(io_kitchen, bit)
                .thenDeviceInputBitIs(io_kitchen, bit, 0, timeout=3.0)

    )

def test_when_multiple_inputs_change_at_the_same_time(rtu_sim_client, mqtt_watcher,io_kitchen,di_test16_in):
    
    s = given(rtu_sim_client, mqtt_watcher)

    (
        s.device(io_kitchen).with_inputs([0]*8).done()
        .device(di_test16_in).with_inputs([0]*16).done()
            .resync() 
            .wait_for_expected_initial_states()
                .when_input_is_set_on(io_kitchen, 2)
                .when_input_is_set_on(di_test16_in, 9)
                .when_input_is_set_on(io_kitchen, 4)
                .when_input_is_set_on(di_test16_in, 12)
                .when_input_is_set_on(di_test16_in, 1)
                .thenDeviceInputIs(io_kitchen, [0,0,0,1,0,1,0,0], timeout=3.0)
