...
- **zigbee** — Zigbee2MQTT driver. Transport (`zigbee_transport.go`) subscribes to Z2M MQTT topics via raw (unprefixed) subscriptions, auto-discovers devices from `bridge/devices`, filters state by edge config + ResourceMap, rounds analog values by `decimalPrecision`, caches blobs for replay after ResourceMap is built. Detects action events from Z2M `action` property and emits them via `ActionEventEmitter` interface; sibling `action_*` fields (e.g. `action_duration`) are collected as metadata via the `eventProperties` map. MQTT retained flag is checked to filter out stale actions on reconnect. Driver (`zigbee_driver.go`) publishes `/set` commands to Z2M, converts bool→ON/OFF for enum state properties. `BypassSignalState() = false`. Configured via `zigbee[].devices[]` in edge config with optional `optimistic` setting per device.
- **uhn** — Core domain types: `DeviceState`, `DeviceCommand`, and interfaces (`EdgePublisher`, `EdgeSubscriber`, `CommandPusher`, `DeviceTransport`, `DeviceDriver`, `ResourceLookup`, `ActionEventEmitter`). `DeviceTransport` manages connection lifecycle; `DeviceDriver` handles protocol-agnostic device interaction (pin type is `any` — numeric for IHC/Modbus, string for Z2M). `ResourceLookup` provides blueprint filtering for transports that need it. `ActionEventEmitter` emits transient action events (button presses) to both the local runtime and MQTT. The `Subscriber` interface's `OnMessage` callback receives a `retained bool` parameter for filtering retained messages.
...
- `uhn/{edgeName}/device/{deviceName}/action/{pin}` — Transient action events (Zigbee buttons)
