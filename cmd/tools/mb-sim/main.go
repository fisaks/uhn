...
func main() {
	...
	// Profile ticker for virtual devices (e.g., Shelly Pro 3EM) — starts disabled
	tickerCtrl := simulator.NewProfileTicker(simStore, 1*time.Second)

	// Single REST API serving all buses
	if err := simulator.StartRestAPI(simStore, restAddr, tcpCtrl, tickerCtrl); err != nil {
